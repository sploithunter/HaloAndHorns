...
local function lockoutActive(t, now)
    return type(t) == "number" and (t < 0 or t > now)
end
...
-- shows a flat ImageLabel (a texture — replicated, CDN-cached, zero per-frame cost). Inventory and
-- trade cards never create ViewportFrames: missing or failed art remains a cheap glyph placeholder.
...
function InventoryPanel.new()
    ...
    local progressionSuccess, progressionResult = pcall(function()
        return ConfigLoader:LoadConfig("player_progression")
    end)
    self.playerProgressionConfig = progressionSuccess and progressionResult or nil
    ...
    self._slotProgressionConnections = {
        self.player:GetAttributeChangedSignal("Level"):Connect(function()
            if self.isVisible and self.equippedGrid then
                self:_updateItemsDisplay()
            end
        end),
        self.player:GetAttributeChangedSignal("PetEquipSlots"):Connect(function()
            if self.isVisible and self.equippedGrid then
                self:_updateItemsDisplay()
            end
        end),
...
function InventoryPanel:_applyCardScale(scaleName)
    ...
    if self.isVisible and self.equippedGrid and self.inventoryGrid then
        local grid = self.equippedGrid:FindFirstChildOfClass("UIGridLayout")
        if grid then
...
function InventoryPanel:Show(parent)
    ...
    if RangeLoadoutSession.isActive() and not self._rangeSession then
        self:BeginRangeCatalog(RangeLoadoutSession.ctx())
    end
...
function InventoryPanel:_applyAvailabilityRing(frame, lockUntil, now)
    ...
    local forever = type(lockUntil) == "number" and lockUntil < 0
    local locked = lockUntil ~= nil
    ring.Visible = true
    ring.Image = locked and PILL_UI.slot_locked or PILL_UI.slot_available -- ruby ring / neutral ring
    timer.Visible = locked and not forever
    timer.Text = (locked and not forever) and lockoutFormatTime(lockUntil - now) or ""
end
...
function InventoryPanel:_renderEmptySlotRings(filledCount)
    ...
    -- Live inventory pads from the replicated slot attribute (0 until it lands). Range
    -- uses the catalog cap so the strip matches the loaned squad, not the player's slots.
    local total = self:_isRangeCatalog() and self:_squadSlotCount()
        or tonumber(self.player and self.player:GetAttribute("PetEquipSlots"))
        or 0
...
-- The next level-earned squad slot is derived from the same progression config used by the server.
-- Game-pass/founder bonus slots do not change this milestone schedule.
function InventoryPanel:_nextPetSlotLevel()
    local pets = self.playerProgressionConfig
        and self.playerProgressionConfig.level_rewards
        and self.playerProgressionConfig.level_rewards.equip_slots
        and self.playerProgressionConfig.level_rewards.equip_slots.pets
    if type(pets) ~= "table" or pets.enabled == false then
        return nil
    end

    local startLevel = math.max(1, math.floor(tonumber(pets.start_level) or 8))
    local everyLevels = math.max(1, math.floor(tonumber(pets.every_levels) or 7))
    local slotsPerMilestone = math.max(1, math.floor(tonumber(pets.slots_per_milestone) or 1))
    local maxBonusSlots = math.max(0, math.floor(tonumber(pets.max_bonus_slots) or 0))
    local maxMilestones = math.ceil(maxBonusSlots / slotsPerMilestone)
    local level = math.max(1, math.floor(tonumber(self.player:GetAttribute("Level")) or 1))
    for milestoneIndex = 0, maxMilestones - 1 do
        local milestone = startLevel + milestoneIndex * everyLevels
...
function InventoryPanel:_renderNextPetSlotHint()
    if self:_isRangeCatalog() then
        return
    end
    local nextLevel = self:_nextPetSlotLevel()
    if not nextLevel or not self.equippedGrid then
        return
    end

    local hint = Instance.new("Frame")
    hint.Name = "NextPetSlotHint"
    hint.Size = UDim2.fromOffset(self.cardSize.X, self.cardSize.Y)
    hint.BackgroundColor3 = Color3.fromRGB(58, 61, 70)
    hint.BackgroundTransparency = 0.08
    hint.BorderSizePixel = 0
    hint.LayoutOrder = self:_squadSlotCount() + 1
    hint.Active = false
    hint.Parent = self.equippedGrid
    ...
    stroke.Color = Color3.fromRGB(125, 129, 143)
    stroke.Thickness = 2
    stroke.Transparency = 0.15
    stroke.Parent = hint

    local label = Instance.new("TextLabel")
    label.Name = "NextPetSlotText"
    label.Size = UDim2.fromScale(0.88, 0.82)
    label.AnchorPoint = Vector2.new(0.5, 0.5)
    label.Position = UDim2.fromScale(0.5, 0.5)
    label.BackgroundTransparency = 1
    label.Font = Enum.Font.GothamBold
    label.Text = "Next pet slot\nLevel " .. tostring(nextLevel)
    label.TextColor3 = Color3.fromRGB(205, 208, 218)
...
function InventoryPanel:_deployedLocks(now)
    ...
    for _, m in ipairs(folder:GetChildren()) do
        if m:IsA("Model") then
            ...
            if lockoutActive(until_, now) then
...
function InventoryPanel:_refreshLockoutVisuals()
    ...
    local depUid, depKey, _claimedSlots = self:_deployedLocks(now)
    local depKeyUsed = {}
    local stackUsed = {}
    local equippedPainted = {}
    for _, frame in ipairs(frames) do
        if frame and frame.Parent then
            ...
            if equipped and kind then
                ...
                table.insert(equippedPainted, { frame = frame, lockUntil = lockUntil })
    ...
    -- Slot locks outlive the draft card. A live downed pet still "claims" its
    -- world slot, but unequipping it from the strip must leave a RED hole (or
    -- keep a replacement card red) for the remaining time.
    local slotTimes = {}
    if map and type(map.slots) == "table" then
        for _, t in pairs(map.slots) do
            if lockoutActive(t, now) then
                slotTimes[#slotTimes + 1] = t
            end
        end
        table.sort(slotTimes, function(a, b)
            return a > b
        end)
    end
    local shown = 0
    for _, row in ipairs(equippedPainted) do
        if row.lockUntil then
            shown += 1
        end
    end
    local leftovers = {}
    for i = shown + 1, #slotTimes do
        leftovers[#leftovers + 1] = slotTimes[i]
    end
    ...
    if rings then
        for i, ring in ipairs(rings) do
            if ring.Parent then
                ringCount += 1
                self:_applyEmptySlotRing(ring, leftovers[i], now)
            end
        end
    end
    local leftoverIdx = ringCount + 1
    for i = #equippedPainted, 1, -1 do
        local row = equippedPainted[i]
        if not row.lockUntil and leftovers[leftoverIdx] then
            self:_applyAvailabilityRing(row.frame, leftovers[leftoverIdx], now)
            leftoverIdx += 1
        end
    end
end
...
function InventoryPanel:_seedDraftFromEquipped()
    if self:_isRangeCatalog() then
        self:_seedDraftFromRangeCatalog()
        return
    end
    local refs = {}
    local player = self.player or Players.LocalPlayer
...
function InventoryPanel:_squadSlotCount()
    if self:_isRangeCatalog() then
        local built = self._rangeSession and self._rangeSession.built
        return (built and built.petSlots) or 5
    end
...
function InventoryPanel:_stampTankLessonGuides()
    ...
    if self.inventoryGrid then
        for _, child in ipairs(self.inventoryGrid:GetDescendants()) do
            if
                child:IsA("Frame")
                and child.Name ~= "VirtualCardSlot"
                and child:GetAttribute("PetRole") == "tank"
            then
                if not best then
                    best = child
                else
...
function InventoryPanel:_strongestInventoryFrame()
    ...
    for _, child in ipairs(self.inventoryGrid:GetDescendants()) do
        if
            child:IsA("Frame")
            and child.Name ~= "VirtualCardSlot"
            and child:GetAttribute("PetRole") ~= nil
        then
            if not best then
                best = child
            else
...
-- Range door: same inventory cards + Best Pets, but the draft is a loaned catalog squad.
function InventoryPanel:BeginRangeCatalog(ctx)
    ctx = ctx or RangeLoadoutSession.ctx()
    ...
    self._rangeSession = {
        ctx = RangeLoadoutSession.ctx() or ctx,
        built = nil,
    }
    self.selectedCategory = "Pets"
    if self.isVisible then
        self:_loadRangeCatalogData()
        self:_seedDraftFromRangeCatalog()
        self:_applyRangeChrome()
        self:_refreshCategoryTabs()
        self:_updateItemsDisplay()
...
function InventoryPanel:_loadRangeCatalogData()
    self.inventoryData = {}
    ...
    local built = RangeCatalogView.build(
        self._rangeSession.ctx.catalog,
        PETS_CONFIG,
        PET_PROGRESSION,
        PetPower
    )
    self._rangeSession.built = built
    self.inventoryData = built.items
end

function InventoryPanel:_seedDraftFromRangeCatalog()
    local session = RangeLoadoutSession.get()
    if session and type(session.pets) == "table" and #session.pets > 0 then
        self._draftRefs = RangeCatalogView.refsFromPicks(session.pets)
    else
        local built = self._rangeSession and self._rangeSession.built
        self._draftRefs = table.clone((built and built.defaultRefs) or {})
    end
    self._draftDirty = false
end

function InventoryPanel:_persistRangePets()
    if not self:_isRangeCatalog() then
        return
    end
    local built = self._rangeSession and self._rangeSession.built
    RangeLoadoutSession.setPets(
        RangeCatalogView.picksFromRefs(built and built.byRef, self._draftRefs)
    )
end

function InventoryPanel:_applyRangeChrome()
    if not (self:_isRangeCatalog() and self.frame) then
        return
    end
    local ctx = self._rangeSession.ctx
    local titleLbl = self.header and self.header:FindFirstChild("HeaderTitle", true)
    if titleLbl and titleLbl:IsA("TextLabel") then
        titleLbl.Text = ctx.display or "The Range"
    end
    if self.searchBox then
        self.searchBox.PlaceholderText = "Search catalog..."
    end
    local invLabel = self.frame:FindFirstChild("InventoryLabel", true)
    if invLabel and invLabel:IsA("TextLabel") then
        invLabel.Text = "Catalog pets"
    end
    if self._activateButton then
        self._activateButton.Text = "✓ Enter"
    end
    local headerButtons = self.header and self.header:FindFirstChild("HeaderButtons")
    if headerButtons then
        for _, child in ipairs(headerButtons:GetChildren()) do
            if child:IsA("GuiButton") and child ~= self._cardScaleButton then
                child.Visible = false
            end
        end
    end
    if self._deletePill then
        self._deletePill.Visible = false
    end
    local categoryContainer = self.content and self.content:FindFirstChild("CategoryContainer")
    if categoryContainer then
        categoryContainer.Visible = false
    end
    self:_rebuildRangeNavBar()
end

function InventoryPanel:_rebuildRangeNavBar()
    if not (self.content and self:_isRangeCatalog()) then
        return
    end
    local existing = self.content:FindFirstChild("RangePowerBar")
    if existing then
        existing:Destroy()
    end

    local bar = Instance.new("Frame")
    bar.Name = "RangePowerBar"
    bar.Size = UDim2.new(0.95, 0, 0.1, 0)
    bar.Position = UDim2.new(0.025, 0, 0.02, 0)
    bar.BackgroundTransparency = 1
    bar.ZIndex = 101
    bar.Parent = self.content
    ...
    hint.Size = UDim2.new(0.62, 0, 1, -2)
    hint.LayoutOrder = 1
    hint.BackgroundTransparency = 1
    hint.Text = "Pets  ·  pick a squad, then match powers"
    ...
    local powers = Instance.new("TextButton")
    powers.Name = "RangePowersButton"
    powers.Size = UDim2.new(0.36, 0, 1, -2)
    powers.LayoutOrder = 2
    powers.Text = "⚡ Powers"
    powers.BackgroundColor3 = Color3.fromRGB(70, 100, 160)
    powers.TextColor3 = Color3.new(1, 1, 1)
    powers.TextScaled = true
    powers.Font = Enum.Font.GothamBold
    powers.ZIndex = 103
    powers.Parent = bar
    pillify(powers)
    powers.Activated:Connect(function()
        self:_persistRangePets()
        RangeLoadoutSession.openPowers()
    end)
end

function InventoryPanel:_enterRangeCatalog()
    if not self:_isRangeCatalog() then
        return
    end
    self:_persistRangePets()
...
function InventoryPanel:_commitDraft(after)
    if self:_isRangeCatalog() then
        self:_enterRangeCatalog()
        return
    end
    if self:_gauntletRosterLocked() then
        self.logger:warn("Squad deploy blocked: gauntlet roster locked")
        return
    end
...
function InventoryPanel:_createItemsGrid()
    ...
    invGridContainer.ClipsDescendants = false
    invGridContainer.LayoutOrder = 4
    invGridContainer.Parent = scrollFrame

    local function updateSectionHeights()
        -- Equipped is deliberately tiny and uses UIGridLayout. Inventory is a virtual grid: its
        -- logical height preserves scrolling while only visible card rows exist as Instances.
        local eqH = eqGrid.AbsoluteContentSize.Y
        local invH = self._virtualInventoryContentHeight or 0
        ...
        -- Section movement changes which logical inventory rows intersect the scrolling window.
        self:_requestCull()
    end
    ...
    self._updateInventorySectionHeights = updateSectionHeights

    -- Initial sizing
    task.defer(updateSectionHeights)

    -- Rebind the bounded card pool whenever the scroll window or grid width changes.
...
function InventoryPanel:_releaseVirtualInventorySlot(index)
    local slots = self._virtualInventorySlotsByIndex
    ...
    if slot.card and self._rightClickConnections then
        local connection = self._rightClickConnections[slot.card]
        if connection then
            connection:Disconnect()
            self._rightClickConnections[slot.card] = nil
        end
    end
    if slot.card then
        for key, frame in pairs(self._stackFrames or {}) do
            if frame == slot.card then
                self._stackFrames[key] = nil
            end
        end
        slot.card:Destroy()
        slot.card = nil
    ...
    table.insert(self._virtualInventoryFreeSlots, slot)
end

function InventoryPanel:_refreshVirtualInventoryWindow()
    local scroll = self.scrollFrame
    local container = self.inventoryGrid
    local items = self._virtualInventoryItems or {}
    if not scroll or not scroll.Parent or not container or not container.Parent then
        return
    end

    local width = container.AbsoluteSize.X
    if width <= 0 then
        -- AbsoluteSize changes already request another reconciliation. Avoid a self-scheduling
        -- defer loop while the panel is hidden or has not completed its first layout pass.
        return
    end
    ...
    if contentHeight ~= self._virtualInventoryContentHeight then
        ...
        if self._updateInventorySectionHeights then
            self._updateInventorySectionHeights()
        end
    end

    self._virtualInventorySlotsByIndex = self._virtualInventorySlotsByIndex or {}
    self._virtualInventoryFreeSlots = self._virtualInventoryFreeSlots or {}
    local slotsByIndex = self._virtualInventorySlotsByIndex
    local viewportHeight = scroll.AbsoluteWindowSize.Y
    if viewportHeight <= 0 then
        viewportHeight = scroll.AbsoluteSize.Y
    end
    local viewportTop = scroll.AbsolutePosition.Y - container.AbsolutePosition.Y
    local range = VirtualGridWindow.visibleRange(
        #items,
        columns,
        self.cardSize.Y,
        self.cardPadding.Y,
    ...
    local releaseIndices = {}
    for index in pairs(slotsByIndex) do
        if not range or index < range.first or index > range.last then
            table.insert(releaseIndices, index)
        end
    end
    for _, index in ipairs(releaseIndices) do
        self:_releaseVirtualInventorySlot(index)
    end

    if range then
        for index = range.first, range.last do
            if not slotsByIndex[index] then
                local slot = table.remove(self._virtualInventoryFreeSlots)
                if not slot then
                    local frame = Instance.new("Frame")
                    frame.Name = "VirtualCardSlot"
                    frame.BackgroundTransparency = 1
                    frame.BorderSizePixel = 0
                    frame.Parent = container
                    slot = { frame = frame }
                end
                slot.frame.Size = UDim2.fromOffset(self.cardSize.X, self.cardSize.Y)
                local x, y = VirtualGridWindow.position(
                    index,
                    columns,
                    self.cardSize.X,
                    self.cardSize.Y,
                ...
                -- Reserve the logical index before card construction. Card assembly can yield on
                -- a config/asset lookup; layout signals may re-enter the deferred refresh during
                -- that yield and must see this index as occupied rather than duplicate it.
                slotsByIndex[index] = slot
                slot.card = self:_createItemFrameInto(items[index], index, slot.frame)
                slot.card.Position = UDim2.fromOffset(0, 0)
            end
        end
    end

    -- Recycled cards are destroyed when they leave the overscan window; keep diagnostics and
    -- lockout scans bounded to live cards rather than retaining dead Instance references.
    local liveFrames = {}
    for _, frame in ipairs(self.itemFrames) do
        if frame.Parent then
            table.insert(liveFrames, frame)
        end
    end
    self.itemFrames = liveFrames

    self:_stampTankLessonGuides()
    self:_stampSquadLessonGuides()
end

-- Compatibility name retained for the handful of existing layout call sites. It now schedules one
-- virtual-window reconciliation rather than walking or rendering the full inventory.
function InventoryPanel:_requestCull()
    ...
    task.defer(function()
        ...
        self:_refreshVirtualInventoryWindow()
    end)
end
...
function InventoryPanel:_loadRealInventoryData()
    ...
    local hoverboardsFolder = inventoryFolder:FindFirstChild("hoverboards")
    if hoverboardsFolder then
        self:_loadHoverboardsFromFolder(hoverboardsFolder)
    end
    ...
    for _, itemFolder in pairs(consumablesFolder:GetChildren()) do
        if itemFolder:IsA("Folder") and itemFolder.Name ~= "Info" then
            ...
            if itemData then
                ...
                local unlockLevel = tonumber(definition.unlock_level)
                local earnedLevel = math.max(
                    1,
                    math.floor(tonumber(Players.LocalPlayer:GetAttribute("Level")) or 1)
                )
                local locked = unlockLevel ~= nil and earnedLevel < unlockLevel
                local baseName = definition.name
                    or itemData.id:gsub("_", " "):gsub("^%l", string.upper)
                local displayData = {
                    id = itemData.id,
                    name = locked and ("🔒 " .. baseName) or baseName,
                    icon = definition.icon or "🧪", -- fallback only for unknown legacy records
                    image = image,
                    description = locked
                            and (definition.locked_description or definition.description)
...
function InventoryPanel:_loadHoverboardsFromFolder(folder)
    for _, child in ipairs(folder:GetChildren()) do
        if child:IsA("Folder") then
            table.insert(self.inventoryData, {
                id = child.Name,
                name = child:GetAttribute("DisplayName") or child.Name,
                image = child:GetAttribute("Icon"),
                icon = "🛹",
                rarity = "Common",
...
-- 🖼️ PET IMAGE ICON CREATION (strictly flat texture with a glyph failure placeholder)
function InventoryPanel:_createPetImageIcon(parent, item)
    ...
    -- flat-image path as every other pet — no special live-ViewportFrame card. A huge is just a
    -- resize; Creator Colorado is huge-sized and frames through this same path.
...
-- Returns "rbxassetid://N" or nil. A missing Huge close-up resolves to the normal flat image.
...
-- Missing thumbnail warnings are deduped. Inventory remains flat-image-only even when a registry
-- entry is absent; that unregistered case uses the card's cheap glyph placeholder instead.
local warnedNoFlatThumb = {}

-- 🔍 GET PET IMAGE FROM ASSETS (Helper function)
-- ONE card path: one flat ImageLabel. No fetch state can create 3D work.
function InventoryPanel:_getPetImageFromAssets(petType, variant, quiet, huge)
    ...
    if thumbId then
        ...
        label.Parent = holder
        if huge then
            -- HUGE is presentation metadata here: zoom the same flat art inside the clipped holder.
    ...
    if not quiet and not warnedNoFlatThumb[warnKey] then
        ...
        self.logger:warn("🚫 Flat pet thumbnail not registered; using glyph placeholder", {
            petType = petType,
            variant = variant,
            huge = huge == true,
        })
...
function InventoryPanel:_updateItemsDisplay()
    ...
    self.itemFrames = {}
    self._stackFrames = {}
    self._virtualInventoryItems = {}
    self._virtualInventorySlotsByIndex = {}
    self._virtualInventoryFreeSlots = {}
    self._virtualInventoryContentHeight = 0
    ...
    if self.equippedLabel then
        ...
        if self:_isRangeCatalog() then
            local best = tonumber(self._rangeSession.ctx.best_room) or 0
            self.equippedLabel.Text = ("Best room %d  ·  Range squad  —  %d/%d"):format(
                best,
                #draftRefs,
            ...
            self.equippedLabel.Text = (
                deployed and "Squad (deployed)  —  " or "Squad (draft)  —  "
            )
                .. #draftRefs
                .. "/"
                .. self:_squadSlotCount()
    ...
    self:_setActivatePulse(self._draftDirty == true or self:_isRangeCatalog())
    ...
    -- Equipped is a tiny ordinary grid. Inventory keeps only a logical list here; the virtual
    -- window below creates cards for visible rows and recycles those slots while scrolling.
    local virtualInventoryItems = {}
    local eqIndex = 1
    for _, item in ipairs(filteredItems) do
        ...
        if isEquipped then
            self:_createItemFrameInto(renderItem, eqIndex, self.equippedGrid)
            eqIndex += 1
        else
            table.insert(virtualInventoryItems, renderItem)
        end
    end
    self._virtualInventoryItems = virtualInventoryItems
    ...
    self:_renderNextPetSlotHint()
    pcall(function()
        self:_refreshLockoutVisuals()
    end)

    -- Bind only the visible logical inventory rows after section layout settles.
    self:_requestCull()
    self:_stampTankLessonGuides()
    self:_stampSquadLessonGuides()
end
...
function InventoryPanel:_createItemFrameInto(item, layoutOrder, parentContainer)
    ...
    -- uiConfig is loaded once when this module is required. Doing a ConfigLoader lookup while
    -- constructing every recycled card can yield and leave a visible slot only half assembled.
    local gridConfig = uiConfig
        and uiConfig.panel_configs
        and uiConfig.panel_configs.inventory_panel
        and uiConfig.panel_configs.inventory_panel.grid
    local configuredIconSize: Vector2? = if gridConfig
            and typeof(gridConfig.icon_size) == "Vector2"
        then gridConfig.icon_size
        else nil
    local configuredIconScale: number? = if gridConfig
            and typeof(gridConfig.icon_scale) == "number"
        then gridConfig.icon_scale
        else nil
    ...
    if item.use3DModel then
        -- Inventory/trade cards are intentionally flat-only. A global viewport preference may
        -- still serve dedicated previews elsewhere, but it cannot turn a large card grid into
        -- hundreds of continuously rendered 3D scenes.
        self:_createPetImageIcon(iconBG, item)
    ...
    if POWER_ICONS and PET_ROLES and PetBadge and item.category == "Pets" and item.petType then
        ...
        if shown > 0 then
            ...
            lbl.Text = (shown > 2 and "ALL")
                or (shown == 2 and (shownLabels[1] .. "+" .. shownLabels[2]))
                or shownLabels[1]
...
function InventoryPanel:_showItemTooltip(item, sourceFrame)
    ...
    if item.locked ~= nil then
        if item.locked and item.unlockLevel then
            table.insert(
                lines,
                { label = "Unlocks", value = "Level " .. tostring(item.unlockLevel) }
            )
        else
...
function InventoryPanel:_addItemInteractions(itemFrame, item)
    ...
    -- Key by the rendered card, not item id: a draft card and an inventory stack card may represent
    -- the same item simultaneously and each owns an independent global UIS listener.
    self._rightClickConnections[itemFrame] = rightClickConnection
...
function InventoryPanel:_handlePrimaryAction(item)
    ...
    elseif item.folder_source == "hoverboards" then
        self:_equipHoverboard(item)
    else
        -- Default: Show info
        self:_showItemInfo(item)
    end
...
function InventoryPanel:_equipHoverboard(item)
    local remote = ReplicatedStorage:FindFirstChild("GameAPICommand")
    if not remote then
        self.logger:warn("❌ GameAPICommand missing for hoverboard equip")
        return
    end
    local ok, envelope = pcall(function()
        return remote:InvokeServer("hoverboard.equip", { skinId = item.id })
    end)
    if not ok or type(envelope) ~= "table" or envelope.ok == false then
        self.logger:warn("❌ Hoverboard equip failed", {
            itemId = item.id,
            error = not ok and tostring(envelope) or (envelope and envelope.code),
        })
        return
    end
    self:_loadRealInventoryData()
...
function InventoryPanel:_isItemEquipped(item)
    ...
    elseif item.folder_source == "hoverboards" then
        return self.player:GetAttribute("HoverboardSkin") == item.id
    end
...
function InventoryPanel:Destroy()
    self:Hide()
    for _, connection in ipairs(self._slotProgressionConnections or {}) do
        connection:Disconnect()
    end
    self._slotProgressionConnections = nil
