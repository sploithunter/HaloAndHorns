-- Walk-up artillery management menu. Same workshop as the bulwark panel, but
...
local MERGE_EGG = require(ReplicatedStorage.Configs:WaitForChild("merge_egg_prototype"))
local MergeCannonPreview = require(ReplicatedStorage.Shared.Game.MergeCannonPreview)
local MergeTowerModels = require(ReplicatedStorage.Shared.Game.MergeTowerModels)

local WORKSHOP_PREVIEW = MergeCannonPreview.settings(MERGE_EGG.team.edge_towers.workshop_preview)
...
local function rgbColor(values)
    return Color3.fromRGB(values[1], values[2], values[3])
end

local function previewClone(familyId, tier)
    local template = MergeTowerModels.GetTemplate(familyId, tier)
    if not template then
        return nil
    end
    local clone = template:Clone()
    clone.Name = "WorkshopPreview"
...
local function clearPreview(slot)
    if slot.model then
        slot.model:Destroy()
        slot.model = nil
    end
...
local function framePreview(slot)
    local model = slot.model
    if not (model and model.Parent) then
        return
    end
    local pixel = slot.viewport.AbsoluteSize
    if pixel.X < 2 or pixel.Y < 2 then
        return
    end
    local boxCF, boxSize = model:GetBoundingBox()
    local lookX, lookZ = MergeCannonPreview.sideLook(
        boxCF.RightVector.X,
        boxCF.RightVector.Z,
        boxCF.LookVector.X,
        boxCF.LookVector.Z,
    ...
    local aabbX, aabbY, aabbZ, centerX, centerY, centerZ = MergeCannonPreview.worldAabbSize(
        boxCF.Position.X,
        boxCF.Position.Y,
        boxCF.Position.Z,
        boxCF.RightVector.X,
    ...
    local visibleWidth, visibleHeight =
        MergeCannonPreview.projectedViewSize(aabbX, aabbY, aabbZ, lookX, lookZ)
    local distance = MergeCannonPreview.fitDistance(
        visibleWidth,
        visibleHeight,
        pixel.X / pixel.Y,
        WORKSHOP_PREVIEW.fovDegrees,
        WORKSHOP_PREVIEW.fill
    )
    local ox, oy, oz = MergeCannonPreview.cameraOffset(lookX, lookZ, distance)
    slot.camera.FieldOfView = WORKSHOP_PREVIEW.fovDegrees
    slot.camera.CFrame = CFrame.lookAt(
        Vector3.new(centerX + ox, centerY + oy, centerZ + oz),
        Vector3.new(centerX, centerY, centerZ),
        Vector3.yAxis
    )
end

-- Viewport fills the existing preview pane. The camera frames that pane
-- so the chassis fills it with padding. Do not inset a smaller window.
local function makePreviewImage(parent, name)
    ...
    viewport.Ambient = rgbColor(WORKSHOP_PREVIEW.ambient)
    viewport.LightColor = rgbColor(WORKSHOP_PREVIEW.lightColor)
    viewport.LightDirection = Vector3.new(
        WORKSHOP_PREVIEW.lightDirection[1],
        WORKSHOP_PREVIEW.lightDirection[2],
        WORKSHOP_PREVIEW.lightDirection[3]
    )
    viewport.ZIndex = 1
    ...
    camera.FieldOfView = WORKSHOP_PREVIEW.fovDegrees
...
local function showPreview(slot, family, tier)
    ...
    local key = string.format("%s:%d", tostring(familyId or ""), resolvedTier)
    if resolvedTier < 1 or familyId == nil then
        slot.key = key
        clearPreview(slot)
        return
    end
    if key == slot.key and slot.model then
        framePreview(slot)
        return
    end
    slot.key = key
    clearPreview(slot)
    local clone = previewClone(familyId, resolvedTier)
    if not clone then
        return
    end
    clone.Parent = slot.world
    slot.model = clone
    framePreview(slot)
end

function MergeCannonMenu.new(parent, onAction)
    local controller = {}
    ...
    screen.Name = "MergeCannonMenuGui"
    ...
    local title = label(titleBar, "Title", "ARTILLERY WORKSHOP", UDim2.new(1, 0, 1, 0), true)
    title.LayoutOrder = 1
    ...
    local selectedName =
        label(selectedHeader, "SelectedName", "CHOOSE A CANNON", UDim2.new(0.58, 0, 1, 0), true)
    ...
    local function makeCard(index)
        ...
        local familyName = label(copy, "FamilyName", "CANNON", UDim2.new(1, 0, 0.56, 0), true)
