...
function MergeBulwarkProgression.actionCost(config, action, family, targetTier)
    config = type(config) == "table" and config or {}
    local id = string.lower(tostring(family or ""))
    assert(FAMILY_SET[id] == true, "Unknown bulwark price family: " .. tostring(family))
    if tostring(action or "") == "unlock" then
        local costs = type(config.unlock_costs) == "table" and config.unlock_costs or {}
        local row = costs[id]
        assert(type(row) == "table", "Missing edge_bulwarks.unlock_costs." .. id)
        assert(
            tonumber(row.amount) ~= nil,
            "Invalid edge_bulwarks.unlock_costs." .. id .. ".amount"
        )
        return {
    ...
    local row = costs[id]
    assert(type(row) == "table", "Missing edge_bulwarks.tier_costs." .. id)
    local amount = tonumber(row[tier])
    assert(amount ~= nil, string.format("Missing edge_bulwarks.tier_costs.%s[%d]", id, tier))
    return {
