...
function Harness:stop()
    self.running = false
    self:_finishProfile("stress_stopped")
...
function Harness:status()
    ...
    return {
        ...
        workers = rows,
        errors = self.errors,
        profile = self:profileStatus(),
    }
end

function Harness:_finishProfile(reason)
    local capture = self.capture
    if not capture or not capture.running then
        return
    end
    capture.running = false
    capture.reason = reason
    capture.elapsed = os.clock() - capture.started
    if capture.connection then
        capture.connection:Disconnect()
        capture.connection = nil
    end
    for _, row in ipairs(capture.originals) do
        if row[1][row[2]] == row[4] then
            row[1][row[2]] = row[3]
        end
    end
    table.clear(capture.originals)
end

function Harness:profileStatus()
    local capture = self.capture
    if not capture then
    ...
    return {
        ...
        elapsed = capture.elapsed or (os.clock() - capture.started),
        counters = capture.counters,
    }
end

-- Inclusive wall time (may contain yielding work), not MicroProfiler CPU attribution.
-- Non-blocking and self-restoring; poll profileStatus instead of holding an MCP call open.
function Harness:profile(selection)
    assert(not self.capture or not self.capture.running, "profile capture already running")
    ...
    -- Validate the whole request before wrapping anything. Bad requests cannot leave
    -- a half-installed capture, and duplicate method names cannot nest wrappers.
    assert(type(selected) == "table", "profile selection must be a table")
    for serviceName, methods in pairs(selected) do
        assert(services[serviceName], "unknown profiling service: " .. tostring(serviceName))
        assert(type(methods) == "table", "profiling methods must be a list")
        local seen = {}
        for _, method in ipairs(methods) do
            assert(type(method) == "string" and not seen[method], "invalid or duplicate method")
            assert(type(services[serviceName][method]) == "function", "unknown method: " .. method)
            seen[method] = true
        end
    end
    self.capture =
        { running = true, started = os.clock(), counters = counters, originals = originals }
    for serviceName, methods in pairs(selected) do
    ...
    self.capture.connection = RunService.Heartbeat:Connect(function()
        if
            os.clock() - self.capture.started >= self.service._config.studio_stress.profile_seconds
        then
            self:_finishProfile("completed")
        end
    end)
    return self:profileStatus()
end

function Harness:start(request)
    assert(not self.running, "stress already running")
    local sources = type(request) == "table" and (request.sources or request)
    ...
    local target = request.occupiedBayTarget or cfg.default_occupied_bays
    assert(type(target) == "number" and target == math.floor(target), "integer bay target required")
    assert(target > 0 and target <= cfg.maximum_occupied_bays, "bay target out of range")
    ...
    table.sort(empty)
    local count = math.min(#empty, math.max(0, target - occupied))
