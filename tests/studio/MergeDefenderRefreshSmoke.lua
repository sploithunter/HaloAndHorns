...
local Smoke = {}
function Smoke.run(Prototype, Enemy, Refresh)
    ...
    local ok, result = pcall(function()
        ...
        assert(not service:_spawnReplacement(record, team, queued, 1))
        assert(
        ...
        active = true
        assert(service:_spawnReplacement(record, team, queued, 1))
        assert(old.Parent == nil and team.config.squad[1] == better)
        ...
        Refresh.step(service, record, team, os.clock() + 10)
        assert(rolls == 1 and team.strongerRefreshRemaining == 0)
        ...
        local enemyService = setmetatable({
            ...
            _dropService = {
                TrySpawnEnhancementDrop = function(_, who, source, position, opts)
                    assert(who == player and source == "enemy" and position == entry.pos)
                    assert(opts.enemy_origin_locked and opts.enemy_element == "ice")
