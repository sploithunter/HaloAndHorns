...
function Smoke.run()
    ...
    local ok, err = pcall(function()
        ...
        n:_stop()
        n:_stepReminders(config.reminder_delay_seconds - 1, false)
        assert(n.current == nil, "Reminder fired early")
        n:_stepReminders(1, false)
        assert(
            n.current and n.current.cue == "tutorial.hatch_first_egg.reminder",
            "Egg reminder missing"
        )
        n:_stop()
        n:_stepReminders(config.reminder_repeat_seconds - 1, false)
        ...
        n:setState({ done = true })
        n:_stop()
        n:_stepReminders(config.reminder_repeat_seconds * 2, false)
        assert(n.current == nil, "Completed tutorial kept reminding")
        n:setState({ id = "hatch_first_egg", index = 1, count = 0 })
        n:_stop()
        n:_stepReminders(config.reminder_repeat_seconds * 2, true)
