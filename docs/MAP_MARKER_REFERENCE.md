...
| `HatcherRing` | `Ring` | none | INFORMATIONAL label on the tower rings around egg hatchers (parts named `HatcherRingTower`, grouped per layer in a `HatcherRing_Lava` model beside EggHatchers). AUTHORED map geometry — never spawned, never flora. **LAVA hatchers only, by design** (Jason 2026-07-16) — 23 towers per ring × 5 layers = 115 parts. Heaven layers = white granite (248,248,248); Home/Hell = dark (91,93,105) SmoothPlastic. `Ring` attr = `<Layer>/Lava`. Canonical source = Home's hand-fixed ring; duplicated to other layers via CFrames relative to the hatcher's base plate (`Meshes/IceysAssetPack_Circle (1)`) — hatcher MODEL pivots are broken (near origin), never use them. The label exists so the 2026-07-16 unlabeled-geometry scripting accident never recurs. |
