...
## Completion audit and alternate access checkpoint — 2026-09-08

The current native map adds a supported northern spectator switchback: two 8-stud-clear 1:8 flights connect ground Y4 to the existing Y12 tier. All 32 Seats, their elevations and the 12-stud Hell fishing passage remain. Sixteen direct movement segments passed with 634 sampled frames and no Jumping/Freefall/Swimming; the navigation tool's off-route shortcut is documented separately. See IMPLEMENTED_ARENA_ACCESS.md.

All 18 occupied fishing proxies fit with no measured body/rod overlap, and both rear-bank loops remained navigable. AS_BUILT.md links the dimensioned architectural sheet. ACCEPTANCE_AUDIT.md retains the exact distinction between standard-body layout proof and concurrent client/device acceptance.

Two complete 17-pass finishing replays passed before the ramp was added and rerun independently as pass 18. Fixed stable-height terrain sampling prevents retained plant grounding drift. Current checkpoint contains 17,695 instances; SHA256 `0eec7c9c25f1fa8f09ed376907b3e17ff3150ae5cb8ddbf82452e858263a725a`. The source validator passes all 18 installers/configs, four supplemental inputs, three runtime clients and 300 paving pieces. Five deliberately broken fixtures are rejected; native round-trip retains Terrain, FX, 18 rods/stations and 16 visitor Seats. Full CI: 2,842/2,842 tests, 322 specs. Temporary world replay backup and export buffer were removed.

The saved artifact is `assets/source/maps/realm_crossroads/RealmCrossroads-R11.rbxl`; the existing R4-named local preview output is refreshed to the same bytes. This does not publish or change production spawn. Final after-view atlas is a separate visual record. Foreground mist/motion, temporal seam observation and target-device performance remain unverified while the host is locked; genuine multiplayer acceptance belongs with gameplay integration.

## Focused desktop follow-up

The next unlocked interval supplied 13 valid short M3 Max rendering conditions: arrival and Bragg at Level04/Level21 with full/reduced/off cosmetic effects, and Heaven Level04/full. Event-frame medians were approximately 16.6–16.8 ms and p95 approximately 19 ms. Three later throttled rows are explicitly rejected; eight other conditions were not reached before the session locked again. This supports those desktop views, not full Hell mist cost or broader device acceptance. Settings/camera were restored and Play stopped. The saved map/hash is unchanged. See FOCUSED_RENDER_QA.md and the opposing-pedestrian addition to OCCUPIED_FISHING_QA.md.
