...
- `PlayerProgressionService` owns config-driven player-level effects. `configs/player_progression.lua` currently contributes a `team_power` modifier from player level and grants extra equipped pet slots at configured level milestones. Inventory slot calculation consumes this service next to permanent upgrades. The service also mirrors authoritative attributes into three presentation-only native People-list values: `Rank` (ordinary earned level, or `⚔️ <veteran>` after the cap), nonblank `Status` (best top-100 leaderboard title when earned, otherwise Noob through Legend, prefixed by VIP/Founder's Legacy icons when owned), and compact `Location` (Home origin, abbreviated realm depth/origin, or Trial). These `leaderstats` values are never saved separately and refresh from the source attributes.
