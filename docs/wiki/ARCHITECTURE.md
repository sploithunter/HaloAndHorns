...
- `WorldTravelService` composes `LayerService` realm movement with `ZoneService` persisted area
  unlocks and `WorldBindingService` map/spawn availability. A persisted unlocked origin is proof the
  player already reached its realm, so World Travel does not reapply first-entry Soul/alignment or
  traversal-token gates (including for an opposite-alignment realm). Activating opens the catalog
  without spending Focus; selecting revalidates the saved unlock before Focus, cooldown, analytics,
  and teleport commit.
...
- `AssetPreloadService` owns imported model normalization for pets. Pet configs can declare `asset_transform.scale`, `asset_transform.huge_scale`, and degree-based `asset_transform.orientation`; normal scale/orientation are baked into the server-only `ServerStorage.Assets.Models` catalog, while huge scale is applied only to owned pets marked with the `huge` trait. Merge copies only the current and next unlocked egg families into the owning player's bounded warm cache; every spawned Workspace pet remains independent of that cache until its authoritative despawn. Because runtime `PivotTo` calls replace a model pivot, `PetFollowController` also composes the stamped orientation attributes at the final local and remote render pivots.
- Pet attack presentation is client-rendered but server-timed. The existing `Combat_PetHit` swing packet starts `PetAttackMotion` role choreography on both the owner and nearby observers; it never decides damage or cadence. The motion is layered over the clean replicated/follow CFrame and before the final imported-model orientation correction, so cosmetic movement cannot feed back into authority or turn authored meshes sideways.
...
- Hatch-time enchant rolls happen through `PetGrantService` after progression slot defaults are stamped. A newly hatched unique pet with an available first slot is guaranteed at least one rolled enchant even when its chance-based rarity profile misses; this is forward-only and does not reconcile existing records. Manual rerolls go through `EnchantService:RerollPetEnchant` and the `EnchantPetRequest`/`EnchantPetResult` remotes. If `configs/enchants.lua` `reroll.requires_station` is true, rerolls require recent activation of a bound `EnchanterStation` map hook.
...
- Equipped unique pet enchants register through the shared `enchants` modifier stage. `EnchantService` interprets the config generically: it reads each effect's `modifier.stage`, `kind`, optional `currency`, `combine`, and `amount_per_strength`, then contributes only when a gameplay system resolves the matching modifier context. The family currency `coins` matches all live biome coin wallets (`grass_coins`, `desert_coins`, `ice_coins`, and `lava_coins`), so Coin Finder is not stranded on a legacy-only context.
- Configured pet variant abilities are executable gameplay, not display metadata. `PetAbilityRuntime` resolves every ability field in `configs/pets.lua`; `PetAbilityService` contributes passive economy, luck, attack-speed, and collection effects through `ModifierService`, while `PetFollowService`, `EnemyService`, `DropService`, and the client follow controller execute combat procs, defense/revival, rare-drop behavior, teleport/flight, and phasing. New ability fields must be added to the runtime resolver and its exhaustive config contract test.
- `PlayerProgressionService` owns config-driven player-level effects. Its initial attribute publication is event-driven from `DataLoaded`; it must never publish unloaded defaults after an arbitrary ProfileStore timeout. `configs/player_progression.lua` currently contributes a `team_power` modifier from player level and grants extra equipped pet slots at configured level milestones. Inventory slot calculation consumes this service next to permanent upgrades. The service also mirrors authoritative attributes into three presentation-only People-list values: `Rank` (ordinary earned level, or `⚔️ <veteran>` after the cap), nonblank `Status` (best top-100 leaderboard title when earned, otherwise the combat-training title Spark → Skilled, otherwise Noob through Legend), and compact `Location` (Home origin, abbreviated realm depth/origin, or Trial). One name badge sits on the custom People list (`configs/people_list.lua`): owner uses the Creator Colorado disc (C + blaster, no ring), then developer, content creator, beta tester, founder (⭐ — Founder's Legacy grants every game pass including VIP, so the star replaces the crown), VIP. The Status chip sits at the 14px top left of the quest bar; click it to wear any earned Training / Adventure / Leaderboard title (`GameData.StatusBadge`). A newly earned option replaces that pick (`GameData.StatusBadgeSeen` is the last published option-key list). A row opens a slide-out card (headshot, how the Status title is earned, Examine Avatar of the live in-game character). Hover sits left of the name column above the quest pill and Status chip. CoreGui PlayerList is disabled. These `leaderstats` values are never saved separately and refresh from the source attributes.
- Progression-gated realm travel resolves access as `max(Level, EffectiveLevel)`: earned progression can never be revoked by a stale derived value or formal-team exemplaring, while sidekicking can still raise access temporarily as the intentional guest pass. `AccessLevel` is shared by the portal, `LayerService`, team follow-warp, and the client lock display so those surfaces cannot disagree.
- `AutoTargetService` owns Phase 5 auto-system settings and server decisions. `configs/auto_systems.lua` declares target modes and hatch auto-delete rules; profiles store only player choices under `Settings.AutoSystems`. Clients request an auto-target attack, but the server selects the breakable and applies the existing breakable attack path. `auto_target.max_target_distance` is also the server-authoritative mining-target retention leash: pets finish assigned crystals only while the owner is outside combat and remains inside it. Entering combat or leaving the radius releases mining immediately. A successfully spawned proximity cave wave also calls `PetFollowService:ReleaseMiningTargets` for players at that cave; this encounter-owned handoff is deliberately separate from ambient realm patrol existence.
...
- Phase 4 now wires the high-priority enchant modifier consumers: `hatch_luck`, `secret_hatch_luck`, `pet_damage`, `team_power`, `pet_efficiency`, and `collect_radius`. `EggService` resolves hatch-luck contexts before selecting the outcome, `PetHandler` resolves team power when spawning equipped pets, the legacy pet follow script resolves damage/cadence modifiers while mining, and `DropService` publishes and consumes the one effective Magnet radius.
...
- `PromoCodeService` is the server-authoritative public-code adapter over `RewardService`. Stable
...
- `AwardDeliveryService` is the producer-neutral offline award boundary. Server producers enqueue a
  ...
  that original deadline. Awards already expired when a producer retries are consumed without being
  queued; when an older queued message is next loaded, the server acknowledges it without granting
  or notifying, removing it from the player's pending ProfileStore message queue.
