...
- Merge player rows, hover, and profile cards now show each player's highest reached wave instead
  of their worn title. Farm & Fight titles, ranks, and name badges are unchanged.
- Added a monotonic durable MergeDefense highest_wave field and replicated per-player
  MergeHighestWave attribute. Existing checkpoints and wave-banner awards provide a
...
- 2026-09-07: Fresh-source eight-bay verification passed: seven isolated workers advanced waves 60→64 and cleaned up without errors. Powers snapshot median 100.5 ms versus the earlier legacy 631 ms; combat ticks remained below the original capture at 17.0 ms versus 22.5 ms. Full CI and production-method regression checks pass. PR #479; not published.

- 2026-09-07: Retired the Ascension/Powers tutorial pointers after player level 8. Both earned and claimed levels gate the entry sign, in-menu coaching, and recommendations; active pointers clear when the level changes. Normal progression and lesson receipts remain intact. Preparing an eight-bay timed server-memory investigation of the reported production UntrackedMemory.
- 2026-09-07: Added deployable, bounded server-memory telemetry in response to aged production screenshots. Samples identify server/version, counter availability, categories, Lua GC, instances and live workload; disabled counters remain unavailable rather than false zeros. No profile writes or client remotes. See [Merge Stress Testing](MERGE_STRESS_TESTING.md).
- 2026-09-07: Native verification confirms advanced-player entry/in-menu cues are cleaned up and title/clipping restored. Deployed sampler boots with tracking enabled and explicit shared-Studio scope. Full CI passes 2,810 tests/318 specs, including disabled-counter handling, bounded history/cadence and partial category failures. PR #480.
- 2026-09-07: The timed eight-bay soak identified 498 destroyed tower projectiles retained by the viewer's unused per-record history. Removed the redundant list in an isolated checkout, preserving the baseline soak. A production-method regression reproduces old-main retention and verifies 100 cleaned projectile lifecycles plus bay-isolated teardown. Full memory-growth attribution remains under investigation; see [Merge Stress Testing](MERGE_STRESS_TESTING.md).
- 2026-09-07: Thirty-minute follow-up found accumulating completed HUD Tweens (157→295→447). Added explicit FillBar Tween/listener cleanup for completion, cancellation, replacement and widget destruction; the production-widget regression fails on old main and passes all lifecycle paths with the fix. Native verification remains scheduled after the unchanged baseline soak; PR #481.
- 2026-09-07: Completed the two-hour eight-bay baseline and over five minutes after fixture teardown. Shared Studio memory grew 7,973→13,727 MB and remained 13,812 MB after models fell 678→34; full attribution remains open. Native projectile and 100-widget HUD cleanup tests passed, with zero retained fixed-module instances. Stopped Play and restored offline config before fresh main validation/publishing; see [Merge Stress Testing](MERGE_STRESS_TESTING.md).

## 2026-09-07 — Limit ordinary offline simulation to two workers
...
- 2026-09-07: Reproduced the mobile defense-line hit-test gap: a ray through the visible deployed egg hit its Body but missed the thin floor-only deployment target. Touch placement now includes owned hatcher eggs and snaps nearby misses to the closest compatible on-screen slot within a config-owned 40-pixel radius. Exact incompatible taps still clear; server inventory/distance/tier checks and camera/multi-touch guards remain authoritative.
- Validation: native two-tap observer routing passed for the visible egg, a 38-pixel near miss beside the egg, and a near miss beside an empty pad; exact tier mismatch sent no request. Substituting the old floor-only target reproduces the failed second tap. Commands were captured locally without profile mutations. Full CI (2,810 tests / 318 specs), standalone picker runtime regression, and wiki checks passed.
...
- 2026-09-08: Started a bounded 12-clip Spanish/Brazilian Portuguese angel/demon listening pilot using the approved voice IDs and settings. Welcome/instruction/reminder samples include optional multi-hatcher advice. No Roblox uploads or runtime locale routing; full production recording remains pending listening review. See TUTORIAL_LOCALIZATION_VOICE_PILOT.md.

- 2026-09-08: Completed the 12-clip localization pilot and four listening reels (about three minutes total). All originals pass technical audio checks; reels match within 0.08 LUFS, and all four load in the listening page. Source text, English meanings, originals, and review reels are retained in draft PR #495 for listening approval; gameplay remains on the existing voice catalog.
