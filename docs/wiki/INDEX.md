...
- [Merge an Egg Prototype](MERGE_EGG_PROTOTYPE.md) — Studio-only Phase 5: one authored land strip,
  four initially empty hatcher positions, independently targeted random egg teams, per-position
  Waycoin-funded Earth→Ice→Lava→Sand progression, an actual-walk-speed balance runner, selectable
  slot/origin-power experiments, a 20-wave ladder, five-egg defeat objective, and stable-slot FIFOs.
