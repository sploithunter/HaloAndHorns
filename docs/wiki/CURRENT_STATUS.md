...
preview manifests. It is the single runtime authority for spawned model identity and for which
chassis the artillery workshop clones. Bulwark workshop cards still use authored `previewDecalId`
art. `scripts/merge_tier_runtime_manifest.json` proves 24 cannon mappings, 24 bulwark mappings,
