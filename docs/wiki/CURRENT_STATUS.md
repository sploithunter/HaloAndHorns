...
- Twelve server notification packets now come from the validated network manifest. Progression and economy slices moved `UpgradeResult`, `LevelUp_Claimed`, `LevelUp_OpenChoice`, `ZoneUnlockResult`, `ZoneTravelResult`, `CurrencyUpdate`, `PurchaseSuccess`, `SellSuccess`, and `EconomyError` without changing wire names or payload tuples; 86 legacy constructors remain under the architecture ratchet.
