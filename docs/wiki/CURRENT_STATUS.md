...
- **Build your squad** and **Power up Resonance** use that same large, bobbing `CLICK HERE`
  treatment on the live Pets and Powers buttons. The cue is parented to the resolved button, so it
  follows scaled/mobile layouts instead of relying on screen coordinates.
