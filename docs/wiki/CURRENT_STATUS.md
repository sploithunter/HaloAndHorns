...
- Sixteen server notification packets now come from the validated network manifest. Progression, economy, realm travel, enchanting, and admin-result slices preserve existing wire names and payload tuples; 82 legacy constructors remain under the architecture ratchet.
