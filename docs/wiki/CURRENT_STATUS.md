...
- Eight server notification packets now come from the validated network manifest. The progression slice moved `UpgradeResult`, `LevelUp_Claimed`, `LevelUp_OpenChoice`, `ZoneUnlockResult`, and `ZoneTravelResult` without changing wire names or payload tuples; 90 legacy constructors remain under the architecture ratchet.
