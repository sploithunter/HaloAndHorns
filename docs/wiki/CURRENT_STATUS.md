...
- Enchant magnitude is resolved through shared `EnchantRuntime` logic for gameplay and UI, including rarity/Huge type scaling. Every configured equipped enchant publishes its own player-bar badge; Active Buffs composes the live modifier pipeline into Attack (Tactics), Pet Speed (Efficiency), Coin rewards, Luck, and Crystal rewards (Home World/Crystal Finder) instead of displaying partial or generic aggregates.
- `Home World` is mechanically live today as an unrestricted `breakable_reward` modifier. Its description still promises a current-world/usefulness condition that the runtime does not yet receive or enforce; that semantic balance rule remains open rather than being silently changed with the HUD repair.
...
- The full-screen Inventory temporarily disables Roblox's `PlayerList` CoreGui so its close button cannot be covered by the expanded People list. Closing, toggling, switching panels, or destroying Inventory restores the exact enabled state captured when Inventory opened.
